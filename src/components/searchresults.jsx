...
const SearchResults = ({ query, state, selected, results, onResultClick }) => {
  ...
  return (
    <div className='search-results'>
      <ul className='search-results-list'>
        {models.map(model =>
          <Card key={model.ekn_id} selected={selected==model.ekn_id} model={model} onClick={() => onResultClick(model)} />
        )}
      </ul>
    </div>
  )
